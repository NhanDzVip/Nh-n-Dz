-- Phát nhạc khi mã chạy (ID nhạc khi chạy mã là 6679600554)
local joinSound = Instance.new("Sound")
joinSound.Name = "JoinSound"
joinSound.SoundId = "rbxassetid://6679600554" -- ID nhạc khi chạy mã
joinSound.Volume = 9
...
require(game.ReplicatedStorage:WaitForChild("Notification")).new(
            " <Color=Red>Nhân Dz On Top<Color=/> "
        ):Display()

require(game.ReplicatedStorage:WaitForChild("Notification")).new(
            " <Color=Green>Welcome to Nhân Dz Community<Color=/> "
        ):Display()

require(game.ReplicatedStorage:WaitForChild("Notification")).new(
            " <Color=Yellow>https://discord.gg/nhwUXhzd<Color=/> "
        ):Display()
		
local MaxSpeed = 500 -- Studs per second 400 no flag but kick
